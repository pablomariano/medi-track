        ...
        foreach ($configuraciones as $config) {
            ...
            if ($tratamiento && $medicamento) {
                DB::table('medicamentos_tratamientos')->insertOrIgnore([
                    ...
                    'tipo_esquema' => $config['es_prn'] ? 'PRN' : 'Programado',
                    'dosis_cantidad' => $this->extractNumericDose($config['dosis']),
                    'unidad_dosis' => $this->extractDoseUnit($config['dosis']),
                    'unidad_dosis_id' => 1, // mg por defecto
                    'frecuencia_horas' => $config['frecuencia_horas'],
                    'tolerancia_antes_minutos' => $config['tolerancia_minutos'],
                    'tolerancia_despues_minutos' => $config['tolerancia_minutos'],
                    'intervalo_minimo_horas' => $config['frecuencia_horas'],
                    'dosis_maxima_dia' => $config['max_dosis_dia'] ?? null,
                    'dosis_diaria_total' => isset($config['max_dosis_dia']) && $config['max_dosis_dia'] ? ($this->extractNumericDose($config['dosis']) * $config['max_dosis_dia']) : null,
                    'duracion_dias' => $config['duracion_dias'],
                    'fecha_inicio' => now()->format('Y-m-d'),
                    'fecha_fin' => now()->addDays($config['duracion_dias'])->format('Y-m-d'),
                    'indicaciones_uso' => $config['instrucciones'],
